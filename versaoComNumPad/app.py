...
os.makedirs(os.path.join(app.static_folder, 'style'), exist_ok=True)
os.makedirs(os.path.join(app.static_folder, 'scripts'), exist_ok=True)
...
def acessibilidade():
    """Página de guia de acessibilidade."""
...
def home():
    ...
    # Cálculo dos timestamps para priorização
    hoje_ts = int(datetime.combine(date.today(), datetime.min.time()).timestamp())
    quinze_dias_ts = hoje_ts + (15 * 24 * 60 * 60)  # 15 dias em segundos
    
    # Ordenação inteligente: prioriza produtos próximos da validade por padrão
    if ordenar == 'nome':
        order_by = 'ORDER BY produto_nome ASC'
    elif ordenar == 'quantidade':
        order_by = 'ORDER BY quantidade DESC'
    elif ordenar == 'validade':
        # Quando explicitamente filtrado por validade, ordena do mais próximo ao mais distante
        order_by = 'ORDER BY validade_int ASC'
    else:
        # Padrão: prioriza produtos próximos da validade (expirados ou dentro de 15 dias)
        # Usa CASE para criar prioridade: 1 = expirado, 2 = próximo (15 dias), 3 = ok
        # Depois ordena por validade dentro de cada grupo
        order_by = f'''ORDER BY 
            CASE 
                WHEN validade_int < {hoje_ts} THEN 1
                WHEN validade_int <= {quinze_dias_ts} THEN 2
                ELSE 3
            END ASC,
            validade_int ASC'''
    ...
    # Processa produtos para adicionar status de validade
    # (hoje_ts e quinze_dias_ts já foram calculados acima)
    produtos = []
    
    for p in produtos_raw:
        produto_dict = dict(p)
        validade_ts = produto_dict['validade_int']
        
        # Determina o status de validade
        if validade_ts < hoje_ts:
            produto_dict['validade_status'] = 'expirado'
        elif validade_ts <= quinze_dias_ts:
            produto_dict['validade_status'] = 'proximo'
        else:
            produto_dict['validade_status'] = 'ok'
...
if __name__ == '__main__':
    ...
    # Verifica dependências opcionmente (controle por variável de ambiente)
    try:
        import os, sys
        # Função utilitária para checar/instalar requirements em tempo de execução
        def ensure_requirements(requirements_file='requirements.txt', auto_install=False):
            """Verifica se os pacotes listados em requirements_file estão presentes.
            Se auto_install=True tenta instalar os que estiverem faltando via pip.

            Retorna um dicionário com o resultado: {'ok': bool, 'missing': [...], 'installed': [...]}.
            """
            import importlib.util, subprocess
            result = {'ok': True, 'missing': [], 'installed': []}
            if not os.path.exists(requirements_file):
                result['ok'] = False
                result['reason'] = f"requirements file not found: {requirements_file}"
                return result
            ...
            if result['missing'] and auto_install:
                cmd = [sys.executable, '-m', 'pip', 'install'] + result['missing']
                try:
        ...
        # Executa automaticamente apenas se a variável de ambiente AUTO_INSTALL_DEPS estiver como '1'
        if os.environ.get('AUTO_INSTALL_DEPS') == '1':
            check = ensure_requirements('requirements.txt', auto_install=True)
            if not check.get('ok'):
                app.logger.warning('ensure_requirements: %s', check)
    except Exception as e:
        app.logger.debug('Erro ao verificar dependências automaticamente: %s', e)
